    ...
    virtual void _appendField(StrSlice slice, size_t field_num, ColumnSet* columns) {
        ...
        switch (column->getType()) {
            case ColumnType::STRING:
                ...
                dynamic_cast<StringColumnSorer*>(column)->append(slice.toCString());
                break;
            case ColumnType::INTEGER:
                dynamic_cast<IntegerColumn*>(column)->append(slice.toInt());
                break;
            case ColumnType::DOUBLE:
                dynamic_cast<DoubleColumnSorer*>(column)->append(slice.toDouble());
                break;
            case ColumnType::BOOL:
                dynamic_cast<BoolColumnSorer*>(column)->append(slice.toInt() == 1);
                break;
            default:
                assert(false);
        }
