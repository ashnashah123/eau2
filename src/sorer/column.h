...
class BaseColumn : public Object {
   public:
    ...
    virtual ~BaseColumn() {
        fprintf(stderr, "basecolumn destructor getting called\n");
 ...
 * Expects ownership of each appended string to be transferred to the StringColumnSorer.
 * Frees contained strings in the destructor.
 */
class StringColumnSorer : public BaseColumn {
   public:
    const char** _entries;
    StringColumnSorer() : BaseColumn(ColumnType::STRING, DEFAULT_CAPACITY) {
        _entries = new const char*[_capacity];
    }

    virtual ~StringColumnSorer() {
...
/**
 * Subclass of BaseColumn holding doubles.
 */
class DoubleColumnSorer : public BaseColumn {
   public:
    double* _entries;
    DoubleColumnSorer() : BaseColumn(ColumnType::DOUBLE, DEFAULT_CAPACITY) {
        _entries = new double[_capacity];
    }
    virtual ~DoubleColumnSorer() { delete[] _entries; }

    virtual void _resize_entries(size_t new_cap) {
        _resize_entry_present(new_cap);
        double* new_entries = new double[new_cap];
        memcpy(new_entries, _entries, _capacity * sizeof(double));
        delete[] _entries;
        _entries = new_entries;
    }

    virtual void append(double entry) {
        _resize_if_necessary();
        _set_entry_present(_length, true);
    ...
    virtual double getEntry(size_t which) {
        assert(which < _length);
        assert(isEntryPresent(which));
        return _entries[which];
    }
 ...
 */
class BoolColumnSorer : public BaseColumn {
   public:
    bool* _entries;
    BoolColumnSorer() : BaseColumn(ColumnType::BOOL, DEFAULT_CAPACITY) {
        _entries = new bool[_capacity];
    }
    virtual ~BoolColumnSorer() { delete[] _entries; }
...
BaseColumn* makeColumnFromType(ColumnType type) {
    switch (type) {
        case ColumnType::STRING:
            return new StringColumnSorer();
        case ColumnType::INTEGER:
            return new IntegerColumn();
        case ColumnType::DOUBLE:
            return new DoubleColumnSorer();
        case ColumnType::BOOL:
            return new BoolColumnSorer();
        default:
            assert(false);
    }
}
...
class ColumnSet : public Object {
   public:
     ...
     */
    virtual ~ColumnSet() {
        fprintf(stderr, "columnset destructor getting called\n");
        for (size_t i = 0; i < _length; i++) {
